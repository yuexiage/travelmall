...
if($_W['isajax']){
    if($op=='selcode'){
        $code       = $_GPC['code'];
        $msg = pdo_fetch("SELECT * FROM ".tablename('yuexiage_travelmall_member_code')." WHERE code='{$code}' and uid='{$uid}' and uniacid = '{$_W['uniacid']}' ");

        if($msg){
            echo  json_encode(['code'=>'1','data','msg'=>'号码正确']);
        }else{
            echo  json_encode(['code'=>'0','data','msg'=>'号码错误']);
        }
        exit;
    }
    ...
    $rid      = $_GPC['rid'];
    $ucid      = $_GPC['ucid'];
    $sql = "SELECT * FROM ".tablename('yuexiage_travelmall_offered')." WHERE id = :id and uniacid = :uniacid";
...
if($op=='join'){
    ...
    if(!$offered['enabled']){
        ...
        if($offered['lower_shelf']<time()){
            ...
            pdo_update('yuexiage_travelmall_offered', $user_data, array('id' => $_GPC['id']));
            include $this->template('saveorder');
            exit;
        }
    }
    ...
    cache_delete('spike_order'.SEP.$_GPC['id'].SEP.$uid);
    ...
    if($save_order){
        ...
        //更新库存
        $sql = "SELECT * FROM ".tablename('yuexiage_travelmall_offered_amount')." WHERE offered_id = :offered_id and  uniacid = :uniacid and start_date= :start_date ";
        ...
        pdo_update('yuexiage_travelmall_offered_amount',$stock, array('offered_id' => $_GPC['id'],'uniacid'=>$_W['uniacid'],'start_date'=>strtotime($_GPC['event_day'])));
    ...
    $sql = "SELECT *,FROM_UNIXTIME(start_date,'%m-%d-%Y') as dat FROM ".tablename('yuexiage_travelmall_offered_amount')." WHERE offered_id = :offered_id and uniacid = :uniacid and stock>0 and start_date>".time();
    $amounts = pdo_fetchall($sql, array(':offered_id'=>$id,':uniacid' => $_W['uniacid']));
    foreach ($amounts as $key => $value) {
        $json[$value['dat']] = "<div class='price_day' id='".$value['id']."' stock='".$value['stock']."' adult='".$value['adult_price']."' child='".$value['child_price']."'>".$value['adult_price']."<br></div>";
    }
